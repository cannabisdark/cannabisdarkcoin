...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new CannabisDarkcoin address for receiving payments.  "
    ...
    return CCannabisDarkcoinAddress(keyID).ToString();
}


CCannabisDarkcoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CCannabisDarkcoinAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current CannabisDarkcoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <CannabisDarkcoinaddress> <account>\n"
            "Sets the account associated with the given address.");

    CCannabisDarkcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid CannabisDarkcoin address");
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount <CannabisDarkcoinaddress>\n"
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CCannabisDarkcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CCannabisDarkcoinAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <CannabisDarkcoinaddress> <amount> [comment] [comment-to]\n"
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CCannabisDarkcoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CCannabisDarkcoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CCannabisDarkcoinAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <CannabisDarkcoinaddress> <message>\n"
    ...
    CCannabisDarkcoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <CannabisDarkcoinaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <CannabisDarkcoinaddress> [minconf=1]\n"
            "Returns the total amount received by <CannabisDarkcoinaddress> in transactions with at least [minconf] confirmations.");

    // CannabisDarkcoin address
    CCannabisDarkcoinAddress address = CCannabisDarkcoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid CannabisDarkcoin address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <toCannabisDarkcoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());

    string strAccount = AccountFromValue(params[0]);
    CCannabisDarkcoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid CannabisDarkcoin address");
    int64_t nAmount = AmountFromValue(params[2]);
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    set<CCannabisDarkcoinAddress> setAddress;
    vector<pair<CScript, int64_t> > vecSend;

    int64_t totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CCannabisDarkcoinAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid CannabisDarkcoin address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a CannabisDarkcoin address or hex-encoded public key\n"
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: CannabisDarkcoin address and we have full public key:
        CCannabisDarkcoinAddress address(ks);
        if (address.IsValid())
        {
            CKeyID keyID;
    ...
    return CCannabisDarkcoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CCannabisDarkcoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CCannabisDarkcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CCannabisDarkcoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("CannabisDarkcoin-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("CannabisDarkcoin-lock-wa");
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // cannabisdarkcoin: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; CannabisDarkcoin server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
public:
    ...
    Object operator()(const CScriptID &scriptID) const {
        ...
        BOOST_FOREACH(const CTxDestination& addr, addresses)
            a.push_back(CCannabisDarkcoinAddress(addr).ToString());
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <CannabisDarkcoinaddress>\n"
            "Return information about <CannabisDarkcoinaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <CannabisDarkcoinpubkey>\n"
            "Return information about <CannabisDarkcoinpubkey>.");
...
// cannabisdarkcoin: reserve balance from being staked for network protection
...
// cannabisdarkcoin: check wallet integrity
...
// cannabisdarkcoin: repair wallet
...
// CannabisDarkcoin: resend unconfirmed wallet transactions
...
// cannabisdarkcoin: make a public-private key pair
