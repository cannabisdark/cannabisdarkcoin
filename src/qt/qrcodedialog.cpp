...
{
    QString ret = QString("CannabisDarkcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CND unit input in lnReqAmount, we generate the URI with CND as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(CannabisDarkcoinUnits::format(CannabisDarkcoinUnits::CND, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
