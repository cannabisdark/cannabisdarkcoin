...
QVariant AddressTableModel::data(const QModelIndex &index, int role) const
{
    ...
    else if (role == Qt::FontRole)
    {
        ...
        {
            font = GUIUtil::cannabisdarkcoinAddressFont();
        }
        return font;
    }
...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case Label:
            ...
            {
                wallet->SetAddressBookName(CCannabisDarkcoinAddress(strTemp).Get(), value.toString().toStdString());
            }
        ...
        case Address:
            ...
            if(CCannabisDarkcoinAddress(rec->address.toStdString()) == CCannabisDarkcoinAddress(value.toString().toStdString()))
            {
                editStatus = NO_CHANGES;
                return false;
            }
            ...
            else if(wallet->mapAddressBook.count(CCannabisDarkcoinAddress(value.toString().toStdString()).Get()))
            {
            ...
            {
                {
                    LOCK(wallet->cs_wallet);
                    // Remove old entry
                    wallet->DelAddressBookName(CCannabisDarkcoinAddress(rec->address.toStdString()).Get());
                    // Add new entry with new address
                    wallet->SetAddressBookName(CCannabisDarkcoinAddress(value.toString().toStdString()).Get(), rec->label.toStdString());
                }
            }
            break;
        }
...
{
    // Update address book model from CannabisDarkcoin core
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address, int addressType)
{
    ...
    if (type == Send)
    {
        ...
        } else
        {
            ...
            {
                LOCK(wallet->cs_wallet);
                if (wallet->mapAddressBook.count(CCannabisDarkcoinAddress(strAddress).Get()))
                {
                    editStatus = DUPLICATE_ADDRESS;
                    return QString();
                };
                
                wallet->SetAddressBookName(CCannabisDarkcoinAddress(strAddress).Get(), strLabel);
        }
    }
    }else if(type == Receive)
    {
        ...
        } else
        {
            ...
            strAddress = CCannabisDarkcoinAddress(newKey.GetID()).ToString();
...
QString AddressTableModel::labelForAddress(const QString &address) const
{
    {      
        ...
        {
            CCannabisDarkcoinAddress address_parsed(sAddr);
