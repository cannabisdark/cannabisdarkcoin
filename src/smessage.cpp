...
int SecureMsgAddWalletAddresses()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(CTxDestination, std::string)& entry, pwalletMain->mapAddressBook)
    {
        ...
        CCannabisDarkcoinAddress coinAddress(entry.first);
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            while (pc < pend)
            {
                ...
                if (opcode == 33)
                {
                    ...
                    CCannabisDarkcoinAddress coinAddress(addressRet);
                    CKeyID hashKey;
...
int SecureMsgScanMessage(unsigned char *pHeader, unsigned char *pPayload, uint32_t nPayload, bool reportToGui)
{
    ...
    */
    ...
    for (std::vector<SecMsgAddress>::iterator it = smsgAddresses.begin(); it != smsgAddresses.end(); ++it)
    {
        ...
        CCannabisDarkcoinAddress coinAddress(it->sAddress);
...
int SecureMsgAddAddress(std::string& address, std::string& publicKey)
{
    ...
    */
    ...
    CCannabisDarkcoinAddress coinAddress(address);
    ...
    CCannabisDarkcoinAddress addressT(address);
...
int SecureMsgEncrypt(SecureMessage& smsg, std::string& addressFrom, std::string& addressTo, std::string& message)
{
    ...
    */
    ...
    bool fSendAnonymous;
    CCannabisDarkcoinAddress coinAddrFrom;
    CKeyID ckidFrom;
    CKey keyFrom;
    ...
    CCannabisDarkcoinAddress coinAddrDest;
    CKeyID ckidDest;
    ...
    } else
    {
        ...
        vchPayload[0] = (static_cast<CCannabisDarkcoinAddress_B*>(&coinAddrFrom))->getVersion(); // vchPayload[0] = coinAddrDest.nVersion;
...
int SecureMsgSend(std::string& addressFrom, std::string& addressTo, std::string& message, std::string& sError)
{
    ...
    */
    ...
    CCannabisDarkcoinAddress coinAddrOutbox;
    ...
    BOOST_FOREACH(const PAIRTYPE(CTxDestination, std::string)& entry, pwalletMain->mapAddressBook)
    {
        ...
        const CCannabisDarkcoinAddress& address = entry.first;
...
int SecureMsgDecrypt(bool fTestOnly, std::string& address, unsigned char *pHeader, unsigned char *pPayload, uint32_t nPayload, MessageData& msg)
{
    ...
    */
    ...
    } else
    {
        ...
        CCannabisDarkcoinAddress coinAddrFromSig;
