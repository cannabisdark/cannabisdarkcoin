...
BOOST_AUTO_TEST_CASE(base58_keys_valid_parse)
{
    ...
    std::vector<unsigned char> result;
    CCannabisDarkcoinSecret secret;
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if(isPrivkey)
        {
            ...
            // Note: CCannabisDarkcoinSecret::SetString tests isValid, whereas CCannabisDarkcoinAddress does not!
...
BOOST_AUTO_TEST_CASE(base58_keys_valid_gen)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        else
        {
            ...
            CCannabisDarkcoinAddress addrOut;
            BOOST_CHECK_MESSAGE(boost::apply_visitor(CCannabisDarkcoinAddressVisitor(&addrOut), dest), "encode dest: " + strTest);
            BOOST_CHECK_MESSAGE(addrOut.ToString() == exp_base58string, "mismatch: " + strTest);
        }
    }

    // Visiting a CNoDestination must fail
    CCannabisDarkcoinAddress dummyAddr;
    CTxDestination nodest = CNoDestination();
    BOOST_CHECK(!boost::apply_visitor(CCannabisDarkcoinAddressVisitor(&dummyAddr), nodest));
